...
class FAQGroup extends Widget {
	...
	public function __construct( $ID, $key, $type = null ) {
		$this->title = get_post_meta($ID, $type ? $type . '_widgets_' . $key . '_title' : 'widgets_' . $key . '_title', true);
		$this->colorScheme = get_post_meta($ID, $type ? $type . '_widgets_' . $key . '_faq_color_scheme' : 'widgets_' . $key . '_faq_color_scheme', true);
		$rowCount = get_post_meta($ID, $type ? $type . '_widgets_' . $key . '_faq' : 'widgets_' . $key . '_faq', true);
		...
		for ($i=0; $i < $rowCount; $i++) {
			$question = get_post_meta($ID, $type ? $type . '_widgets_' . $key . '_faq_'.$i.'_field_the_question' : 'widgets_' . $key . '_faq_'.$i.'_field_the_question', true);
			$answer = get_post_meta($ID, $type ? $type . '_widgets_' . $key . '_faq_'.$i.'_field_the_answer' : 'widgets_' . $key . '_faq_'.$i.'_field_the_answer', true);
